class Node:
    ...
    def search(self, word, i):
        if not self.val and len(word) == i:
            return self.is_word
        ...
        for idx, char in enumerate(self.val):
            ...
            if word[i + idx] != char:
                return False
            if i + idx == len(word) - 1 and idx == len(self.val) - 1:
                return self.is_word
        next_letter = word[i + len(self.val)]
        if next_letter not in self.children:
            return False
        nextt = self.children.get(next_letter)
        return nextt.search(word, i + len(self.val) + 1)

    def prefixSearch(self, word, i):
        if len(word) == i:
            return True
        for idx, char in enumerate(self.val):
            if i + idx == len(word):
        ...
        if next_letter not in self.children:
            return False
        nextt = self.children.get(next_letter)
        return nextt.prefixSearch(word, i + len(self.val) + 1)
